...
<script>
    //添加页面
    function addItems() {
        location.href="${pageContext.request.contextPath}/addUse";
    }
        ...
        if($('#cusTable').bootstrapTable('getSelections').length<1){
            ...
            BootstrapDialog.show({
                ...
                buttons:[{label:'关闭',cssClass : "btn-primary", action : function(dialog){   //给当前按钮添加点击事件
                        ...
                        dialog.close();
                        $.post('deleteUse',{'ids':ids},function (data) {
                            alert("删除成功");
                            location.href = "useManager";
                        })
                    }}
                ]
            });
    ...
    $(function () {
        ...
        $('#cusTable').bootstrapTable({
            ...
            sortable: true,
            silentSort:true,
            url: '${pageContext.request.contextPath}/useList',
            striped: true,
            ...
            columns: [
                ...
                },
                {
                    ...
                    field: 'status', title: '用途状态', formatter: function (v, r, i) {
                        switch (v) {
                            case 1:
                                return '正常';
                                break;
                    ...
                    formatter: function (value, row, index) {
                        return ["<a class='btn btn-default' href='modifyUse/" + row.id + "'>编辑</a>",
